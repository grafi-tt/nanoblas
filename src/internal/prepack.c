...
int step_prepack(prepack_state_t *st) {
	...
	if (st->slice_packed_size == st->slice_size) {
		...
		st->mn_len_remained -= st->mn_slice_real_len;
		st->mn_slice_real_len = imin(st->mn_slice_virtual_len, st->mn_len_remained);
	}
	st->sched_size = imin(st->max_sched_size, st->slice_size - st->slice_packed_size);
	return retval;
}
...
void pack_all(prepack_state_t *st) {
	do {
		do {
			...
			} while (st->mn_slice_pos < st->mn_slice_virtual_len);
			st->mn_slice_pos = 0;
