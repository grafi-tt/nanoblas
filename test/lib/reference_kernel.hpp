...
struct RK {
	static void reference_kernel_fun(nanoblas_kernel_state_t<FTYPE> *kernel_st) {
	...
	static void reference_pack_fun(nanoblas_prepack_state_t<FTYPE> *prepack_st) {
		...
		prepack_st->next_cur += prepack_st->interval_k * prepack_st->sched_len;
		prepack_st->next_pack_cur += prepack_st->slice_len * prepack_st->sched_len;
	}

	static thread_local nanoblas_kernel_t<FTYPE> reference_kernel;
};

template<typename FTYPE>
thread_local nanoblas_kernel_t<FTYPE> RK<FTYPE>::reference_kernel = {
